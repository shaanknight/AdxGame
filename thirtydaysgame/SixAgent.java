 ...
 */
public class SixAgent extends ThirtyDaysThirtyCampaignsAgent {

  public SixAgent(String host, int port) {
    super(host, port);
  }
    ...
    try {
      ...
      for(MarketSegment m2 : MarketSegment.values())
      {
        if(MarketSegment.marketSegmentSubset(c.getMarketSegment(),m2)) {
          bidEntries.add(new SimpleBidEntry(m2, 0.3*c.getBudget() / (double) c.getReach(), 0.25*c.getBudget()));
        }
      }
  ...
  public static void main(String[] args) {
	  SixAgent agent = new SixAgent("localhost", 9898);
    agent.connect("agent6");
  }
