 ...
 */
public class TenAgent extends ThirtyDaysThirtyCampaignsAgent {

  public TenAgent(String host, int port) {
    super(host, port);
  }
    ...
    try {
      ...
      for(MarketSegment m2 : MarketSegment.values())
      {
        if(MarketSegment.marketSegmentSubset(c.getMarketSegment(),m2)) {
          bidEntries.add(new SimpleBidEntry(m2, 0.95*c.getBudget() / (double) c.getReach(), 0.25*c.getBudget()));
        }
      }
  ...
  public static void main(String[] args) {
	  TenAgent agent = new TenAgent("localhost", 9898);
