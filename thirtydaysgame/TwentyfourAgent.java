 ...
 */
public class TwentyfourAgent extends ThirtyDaysThirtyCampaignsAgent {

  public TwentyfourAgent(String host, int port) {
    super(host, port);
  }
  ...
  protected ThirtyDaysBidBundle getBidBundle(int day) {
    try {
      ...
      for(MarketSegment m2 : MarketSegment.values())
      {
        if(MarketSegment.marketSegmentSubset(c.getMarketSegment(),m2)) {
          if(m2 == MarketSegment.MALE_YOUNG_LOW_INCOME)
            bidEntries.add(new SimpleBidEntry(m2, c.getBudget() / (double) c.getReach(), 1.1*(arr[0]/val)*c.getBudget()));
          if(m2 == MarketSegment.MALE_YOUNG_HIGH_INCOME)
            bidEntries.add(new SimpleBidEntry(m2, c.getBudget() / (double) c.getReach(), 1.1*(arr[1]/val)*c.getBudget()));
          if(m2 == MarketSegment.MALE_OLD_LOW_INCOME)
            bidEntries.add(new SimpleBidEntry(m2, c.getBudget() / (double) c.getReach(), 1.1*(arr[2]/val)*c.getBudget()));
          if(m2 == MarketSegment.MALE_OLD_HIGH_INCOME)
            bidEntries.add(new SimpleBidEntry(m2, c.getBudget() / (double) c.getReach(), 1.1*(arr[3]/val)*c.getBudget()));
          if(m2 == MarketSegment.FEMALE_YOUNG_LOW_INCOME)
            bidEntries.add(new SimpleBidEntry(m2, c.getBudget() / (double) c.getReach(), 1.1*(arr[4]/val)*c.getBudget()));
          if(m2 == MarketSegment.FEMALE_YOUNG_HIGH_INCOME)
            bidEntries.add(new SimpleBidEntry(m2, c.getBudget() / (double) c.getReach(), 1.1*(arr[5]/val)*c.getBudget()));
          if(m2 == MarketSegment.FEMALE_OLD_LOW_INCOME)
            bidEntries.add(new SimpleBidEntry(m2, c.getBudget() / (double) c.getReach(), 1.1*(arr[6]/val)*c.getBudget()));
          if(m2 == MarketSegment.FEMALE_OLD_HIGH_INCOME)
            bidEntries.add(new SimpleBidEntry(m2, c.getBudget() / (double) c.getReach(), 1.1*(arr[7]/val)*c.getBudget()));
        }
      }
  ...
  public static void main(String[] args) {
	  TwentyfourAgent agent = new TwentyfourAgent("localhost", 9898);
    agent.connect("agent24");
  }
