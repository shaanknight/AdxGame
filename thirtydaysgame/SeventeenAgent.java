 ...
 */
public class SeventeenAgent extends ThirtyDaysThirtyCampaignsAgent {

  public SeventeenAgent(String host, int port) {
    super(host, port);
  }
  ...
  protected ThirtyDaysBidBundle getBidBundle(int day) {
    try {
      ...
      for(MarketSegment m2 : MarketSegment.values())
      {
        if(MarketSegment.marketSegmentSubset(c.getMarketSegment(),m2)) {
          if(m2 == MarketSegment.MALE_YOUNG_LOW_INCOME)
            bidEntries.add(new SimpleBidEntry(m2, 0.7*c.getBudget() / (double) c.getReach(), (arr[0]/val)*c.getBudget()));
          if(m2 == MarketSegment.MALE_YOUNG_HIGH_INCOME)
            bidEntries.add(new SimpleBidEntry(m2, 0.7*c.getBudget() / (double) c.getReach(), (arr[1]/val)*c.getBudget()));
          if(m2 == MarketSegment.MALE_OLD_LOW_INCOME)
            bidEntries.add(new SimpleBidEntry(m2, 0.7*c.getBudget() / (double) c.getReach(), (arr[2]/val)*c.getBudget()));
          if(m2 == MarketSegment.MALE_OLD_HIGH_INCOME)
            bidEntries.add(new SimpleBidEntry(m2, 0.7*c.getBudget() / (double) c.getReach(), (arr[3]/val)*c.getBudget()));
          if(m2 == MarketSegment.FEMALE_YOUNG_LOW_INCOME)
            bidEntries.add(new SimpleBidEntry(m2, 0.7*c.getBudget() / (double) c.getReach(), (arr[4]/val)*c.getBudget()));
          if(m2 == MarketSegment.FEMALE_YOUNG_HIGH_INCOME)
            bidEntries.add(new SimpleBidEntry(m2, 0.7*c.getBudget() / (double) c.getReach(), (arr[5]/val)*c.getBudget()));
          if(m2 == MarketSegment.FEMALE_OLD_LOW_INCOME)
            bidEntries.add(new SimpleBidEntry(m2, 0.7*c.getBudget() / (double) c.getReach(), (arr[6]/val)*c.getBudget()));
          if(m2 == MarketSegment.FEMALE_OLD_HIGH_INCOME)
            bidEntries.add(new SimpleBidEntry(m2, 0.7*c.getBudget() / (double) c.getReach(), (arr[7]/val)*c.getBudget()));
        }
      }
  ...
  public static void main(String[] args) {
	  SeventeenAgent agent = new SeventeenAgent("localhost", 9898);
    agent.connect("agent17");
  }
